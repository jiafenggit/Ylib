...
ydom_t *ydom_new() {
	...
	if (!(dom = ymalloc(sizeof(ydom_t)))) {
		YLOG_ADD(YLOG_ERR, "Unable to allocate memory");
		return (NULL);
	}
	if (!(node = ymalloc(sizeof(ydom_node_t)))) {
		yfree(dom);
		YLOG_ADD(YLOG_ERR, "Unable to allocate memory");
		return (NULL);
	}
...
void ydom_free(ydom_t *dom) {
	...
	if (dom->xml_version)
		yfree(dom->xml_version);
	if (dom->encoding)
		yfree(dom->encoding);
	if (dom->standalone)
		yfree(dom->standalone);
	yfree(dom);
	YLOG_ADD(YLOG_DEBUG, "Exiting");
}
...
ydom_node_t *ydom_node_add_elem(ydom_node_t *node, char *tagname) {
	...
	if (!node || !(new_node = ymalloc(sizeof(ydom_node_t))))
		return (NULL);
...
ydom_node_t *ydom_node_add_text(ydom_node_t *node, char *data) {
	...
	if (node->last_child && node->last_child->node_type == TEXT_NODE) {
		tmp = ymalloc(strlen(node->last_child->value) + strlen(data) + 1);
		strcpy(tmp, node->last_child->value);
		strcat(tmp, data);
		yfree(node->last_child->value);
		node->last_child->value = tmp;
	} else {
		if (!(text_node = ymalloc(sizeof(ydom_node_t))))
			return (NULL);
...
void ydom_node_rm_children(ydom_node_t *node) {
	...
	while (node->first_child) {
		...
		yfree(node->first_child->name);
		yfree(node->first_child->value);
		pt = node->first_child;
...
static void _ydom_inside_hdlr(ysax_t *sax, char *str) {
	...
	dom = (ydom_t*)YSAX_DATA(sax);
	if (dom->current_parsed_node->node_type == TEXT_NODE) {
		tmp = ymalloc(strlen(dom->current_parsed_node->value) + strlen(str) + 1);
		strcpy(tmp, dom->current_parsed_node->value);
		strcat(tmp, str);
		yfree(str);
		yfree(dom->current_parsed_node->value);
		dom->current_parsed_node->value = tmp;
	} else {
		if (!(text = ymalloc(sizeof(ydom_node_t)))) {
			YLOG_ADD(YLOG_ERR, "Unable to allocate memory");
			return;
		}
	...
	if (!strcmp(target, XML)) {
		if (content && (pt = strstr(content, XML_VERSION))) {
			if ((pt = strchr(pt, EQ)) &&
				...
				if ((dom->xml_version = ymalloc(pt2 - pt))) {
		...
		if (content && (pt = strstr(content, ENCODING))) {
			if ((pt = strchr(pt, EQ)) &&
			    ((pt = strchr(pt, DQUOTE)) || (pt = strchr(pt, QUOTE))) &&
			    ((pt2 = strchr(pt + 1, DQUOTE)) || (pt2 = strchr(pt + 1, QUOTE)))) {
				if ((dom->encoding = ymalloc(pt2 - pt))) {
		...
		if (content && (pt = strstr(content, STANDALONE))) {
			if ((pt = strchr(pt, EQ)) &&
			    ((pt = strchr(pt, DQUOTE)) || (pt = strchr(pt, QUOTE))) &&
			    ((pt2 = strchr(pt + 1, DQUOTE)) || (pt2 = strchr(pt + 1, QUOTE)))) {
				if ((dom->standalone = ymalloc(pt2 - pt))) {
...
static ydom_node_t *_ydom_add_attr_to_node(ydom_node_t *node,
	...
	if (!(attribute = ymalloc(sizeof(ydom_node_t))))
		return (NULL);
