...
static ysax_attr_t *_ysax_add_attrs(yvect_t *attrs, char *attr_name, char *attr_value) {
	...
	if (!attrs || !(attr = ymalloc(sizeof(ysax_attr_t))))
		return (NULL);
	if (!*attrs && !(*attrs = yv_new(0))) {
		yfree(attr);
		return (NULL);
	}
	attr->name = attr_name;
...
static yerr_t _ysax_parse_open_tag(ysax_t *sax) {
	...
	if (!(name_buf = ys_new("")) || !(attr_name_buf = ys_new("")) ||
	    !(attr_value_buf = ys_new("")) || !(attrs = yv_new(0))) {
		YLOG_ADD(YLOG_ERR, "Unable to allocate memory");
		return (YENOMEM);
	}
	while ((c = _ysax_getc(sax)) != EOF) {
		if (!IS_SPACE(c)) {
			if (c == LT) {
				...
				yv_free(&attrs, ysax_free_attr, NULL);
				YLOG_ADD(YLOG_ERR, "Syntax error");
				return (YESYNTAX);
			} else if (c == SLASH) {
...
static yerr_t _ysax_parse_cdata(ysax_t *sax) {
	...
	if ((c = _ysax_getc(sax)) == EOF || c != LSQUAREBRACKET) {
		YLOG_ADD(YLOG_ERR, "Syntax error");
		return (YESYNTAX);
	}
	if (!(content = ys_new(""))) {
	...
	while ((c = _ysax_getc(sax)) != EOF && c != RSQUAREBRACKET)
		ys_addc(&content, c);
	if ((c = _ysax_getc(sax)) == EOF || c != RSQUAREBRACKET ||
